...
# Paths
MODEL_PATH = os.path.join(os.path.dirname(__file__), "..", "backend", "models", "model.joblib")
DATA_PATH = os.path.join(os.path.dirname(__file__), "sample_data", "air_quality_real.csv")
BENCHMARK_OUT = os.path.join(os.path.dirname(__file__), "..", "backend", "models", "benchmark.json")
...
def benchmark_inference_latency(model, X, n_samples=1000):
    ...
    for batch_key, batch_info in batch_times.items():
        print(f"  {batch_key:15} | Total: {batch_info['total_ms']:8.2f} ms | Per-sample: {batch_info['per_sample_ms']:.4f} ms")
...
def benchmark_model_size(model_path):
    ...
    return {
        ...
        'memory_mb': float(model_memory_mb)
    }
...
def main():
    ...
    print("="*60)
    print(f"Mean latency (single):  {latency_results['single_prediction_ms']['mean']:.4f} ms")
    print(f"Throughput:             {throughput_results['throughput_pred_per_sec']:.1f} pred/sec")
